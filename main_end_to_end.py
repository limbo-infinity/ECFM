...
from train import train_end_to_end_q_bid
from virtual_trading_simulator import evaluate_q_bid_policy
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "decision_model": {
            "decision": "raw [q, bid_price] generated by ECFM",
            "q_transform": "q_max * tanh(raw_q)",
            "bid_transform": "low + (up - low) * sigmoid(raw_bid)",
            "clearing": "sigmoid(alpha * q * (bid_price - DA))",
    ...
    end_to_end_optimizer = torch.optim.AdamW(
        list(prediction_model.parameters()) + list(ecfm_model.parameters()),
        lr=end_to_end_learning_rate,
        weight_decay=end_to_end_weight_decay,
    )
    end_to_end_losses = train_end_to_end_q_bid(
    ...
    train_policy_metrics, train_policy_arrays = evaluate_q_bid_policy(
        prediction_model=prediction_model,
        ecfm_model=ecfm_model,
    ...
    val_policy_metrics, val_policy_arrays = evaluate_q_bid_policy(
        prediction_model=prediction_model,
        ecfm_model=ecfm_model,
    ...
    plot_curve(
        ...
        title="End-to-end option-C trading loss",
        ylabel="Energy loss",
        label="End-to-end loss",
        hyperparams=hyperparams,
    )
    ...
    logger.save_npz(
        ...
        end_to_end_loss=np.asarray(end_to_end_losses, dtype=np.float32),
    ...
    metrics = {
        ...
        "final_end_to_end_loss": end_to_end_losses[-1],
    ...
    logger.save_model(
        ...
        ecfm_model,
        extra={
            "model_type": "option_c_ecfm_q_bid",
            "T": T,
            "K": K,
            "decision_dim": 2,
            "hyperparams": hyperparams,
    ...
    print(f"Final end-to-end loss: {end_to_end_losses[-1]:.4f}")
